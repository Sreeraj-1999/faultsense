...
async def train_ml_model(request: dict):
    ...
    try:
        # Parse request
        # ml_data = request['ML_Data_sent']
        # ml_data = request.get('ML_Data_sent') or request
        # if 'ML_Data_sent' in request:
        #     ml_data = request['ML_Data_sent']
        # else:
        #     ml_data = request
        if 'data' in request:
        ...
        logger.info(f"📊 Vessel IMO: {vessel_imo}, Equipment: {equipment}")
        ...
        if nan_cols:
            ...
            X, y, timestamps = create_sequences(df, input_cols, output_cols, 'Local_time', vessel_boundaries, 672, 24, 24)
        ...
        final_model, train_losses, val_losses, best_val_loss = train_model(
            final_model, train_loader, val_loader, criterion, optimizer, scheduler, epochs=200, patience=15, device=device
        )
        
        # Save
        # model_dir = "models/combined_vessels"
        # Save
        model_dir = f"models/{vessel_imo}_{equipment}"
        ...
        return {
            ...
            "sequences": len(X), "input_features": len(input_cols), "output_features": len(output_cols)
        }
    except Exception as e:
