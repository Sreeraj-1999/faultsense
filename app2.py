...
async def train_ml_model(request: dict):
    ...
    try:
        ...
        final_model, train_losses, val_losses, best_val_loss = train_model(
            final_model, train_loader, val_loader, criterion, optimizer, scheduler, epochs=20, patience=15, device=device
        )
        ...
        try:
            from autoencoder2 import train_autoencoder
    
            # Prepare data for autoencoder - use ALL CSV rows (not sequences!)
            logger.info("📦 Preparing ALL CSV rows for autoencoder...")
            
            # Get all rows from df (ALL 17,266 rows!)
            df_rows = df[input_cols].values  # Shape: (17266, 17)
            df_timestamps = df['Local_time'].tolist()
            
            logger.info(f"   Total CSV rows: {len(df_rows)}")
            
            # Split train/val (80/20) - same split as sequencesed
            split_idx = int(0.8 * len(df_rows))
            X_train_rows_unscaled = df_rows[:split_idx]      # (13812, 17)
            X_val_rows_unscaled = df_rows[split_idx:]        # (3454, 17)
            
            timestamps_train_ae = df_timestamps[:split_idx]
            timestamps_val_ae = df_timestamps[split_idx:]
            
            # Create FRESH scaler for autoencoder (don't use TS scaler!)
            logger.info("📊 Creating fresh scaler for autoencoder...")
            ae_input_scaler = StandardScaler()
            X_train_rows = ae_input_scaler.fit_transform(X_train_rows_unscaled)
            X_val_rows = ae_input_scaler.transform(X_val_rows_unscaled)

            logger.info("   ✅ Autoencoder data scaled with fresh scaler")
            
            logger.info(f"   Train rows: {X_train_rows.shape}")  # (13812, 17)
            logger.info(f"   Val rows: {X_val_rows.shape}")      # (3454, 17)
            
            # Train autoencoder with ALL CSV rows!
            autoencoder_results = train_autoencoder(
                X_train_scaled=X_train_rows,      # (13812, 17)
                X_val_scaled=X_val_rows,          # (3454, 17)
                device=device,
                save_dir=model_dir,
                feature_names=input_cols,
                input_scaler=ae_input_scaler,
                timestamps_train=timestamps_train_ae,
                timestamps_val=timestamps_val_ae
            )
        ...
        try:
            ...
            # Use SAME data as autoencoder (ALL CSV rows!)
            all_actual_ffn = np.concatenate([
                X_train_rows_unscaled,  # (13812, 17)
                X_val_rows_unscaled     # (3454, 17)
            ], axis=0)

            logger.info(f"   FFN all_actual_ffn shape: {all_actual_ffn.shape}")  # (17266, 17)

            anomaly_flags_ffn = autoencoder_results['anomaly_detection_overall']['anomaly_flags_full']
            logger.info(f"   FFN anomaly_flags length: {len(anomaly_flags_ffn)}")  # 17266

            # Use autoencoder timestamps
            all_timestamps_ffn = timestamps_train_ae + timestamps_val_ae
...
async def explain_autoencoder_anomalies_endpoint(request: dict):
    ...
    try:
        ...
        # Call SHAP explainer
        from shap_explainer2 import explain_autoencoder_anomalies
        ...
        return results
        # Check if SHAP returned an error (success=false)
        # if not results.get("success", True):
        #     # Return error with outer success=false
        #     return {
        #         "success": False,
        #         "data": results
        #     }
        
        # # Success case - return with outer success=true
        # return {
        #     "success": True,
        #     "data": results
        # }
